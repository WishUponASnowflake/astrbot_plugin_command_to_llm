...
class CommandToLLM(Star):
    def __init__(self, context: Context, config: AstrBotConfig = None):
        ...
        # 初始化数据管理器
        self.data_manager = DataManager(context)
        
        # 初始化指令处理器
        self.command_processor = CommandProcessor(self)
        
        # 初始化动态LLM管理器
        self.dynamic_llm_manager = DynamicLLMManager(context, self.data_manager, self.command_processor)
        
        # 注册动态LLM函数
        self.dynamic_llm_manager.register_dynamic_functions()
        
        logger.info(f"指令转LLM插件启动成功，已加载 {len(self.data_manager.command_mappings)} 个指令映射，注册了 {len(self.dynamic_llm_manager.get_registered_functions())} 个动态LLM函数")
    ...
    @cmd2llm.command("add")
    async def add_mapping(self, event: AstrMessageEvent, command_str: str, llm_function: str, description: str = ""):
        '''添加指令映射
        
        格式：/cmd2llm add <指令名> <LLM函数名> [描述]
        示例：/cmd2llm add rmd--ls list_reminders 列出所有提醒
        '''
        logger.info(f"[command_to_llm] add_mapping 被调用，command_str: '{command_str}', llm_function: '{llm_function}', description: '{description}'")
        
        # 解析指令名（将 -- 替换为空格）
        command_name = command_str.replace("--", " ")
        
        logger.info(f"[command_to_llm] 解析结果 - 指令名: '{command_name}', LLM函数: '{llm_function}', 描述: '{description}'")
        
        try:
            async for result in self.command_processor.add_mapping(event, command_name, llm_function, description):
                yield result
    ...
    @cmd2llm.command("ls")
    async def list_mappings(self, event: AstrMessageEvent):
        '''列出所有指令映射'''
        async for result in self.command_processor.list_mappings(event):
            yield result
    ...
    async def execute_cmd(self, event: AstrMessageEvent, command_str: str, args: str = ""):
        '''执行指令
        ...
        async for result in self.command_processor.execute_command(event, command_text, args):
            yield result
...
"""
    ...
    async def refresh_functions(self, event: AstrMessageEvent):
        ...
        try:
            ...
            yield event.plain_result(f"刷新完成，当前注册了 {registered_count} 个动态LLM函数")
        except Exception as e:
